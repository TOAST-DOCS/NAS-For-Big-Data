...
| アクセス制御リスト(ACL) | Network ACLサービスでアクセス制御リスト(ACL)を設定できます。詳細は[Network ACLサービスユーザーガイド](/Network/Network%20ACL/ko/overview)を参照してください。 |
| スナップショット自動作成 | 毎日指定した時間にスナップショットを自動的に作成します。設定した数を超過すると、最も古いスナップショットから順次削除されます。 |
...
Network ACLサービスでアクセス制御リスト(ACL)を設定できます。詳細は[Network ACLサービスユーザーガイド](/Network/Network%20ACL/ko/overview)を参照してください。
